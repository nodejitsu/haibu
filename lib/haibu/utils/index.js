...
utils.validateForShell = function(validated) {
    isUnsafeForShell = function(value) {
      shell_opeartors_re = new RegExp(/(&{1,2}|\.\/|\|{1,2}|(%.*%)|\$|\.{2}|<{1,2}|>{1,2}|;)/g)
      return shell_opeartors_re.test(value)
    }

    for(key in validated) {
      value = validated[key]
      if(isUnsafeForShell(value)) {
