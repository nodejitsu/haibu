...
//
// ### function rmdir / clean (callback)
// #### @callback {function} Continuation to pass control back to when complete.
// Checks for the existance of `this.appDir` and then removes it if it exists
//
Repository.prototype.rmdir = Repository.prototype.clean = function (callback) {
  var self = this;
  fs.stat(this.appDir, function (err, stats) {
    ...
    exec('rm -rf ' + self.appDir, function (err) {
      if (err) {
        return callback(err);
      }
