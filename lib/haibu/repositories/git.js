...
Git.prototype.init = function (callback) {
  ...
  try {
    utils.validateForShell({'Invalid git url' : self.app.repository.url,
                            'Invalid git branch name' : self.app.repository.branch,
                            'Invalid application dir' : self.appDir,
                            'Invalid home dir' : self.app.directories ? self.app.directories.home : ''});
  }
  catch(error) {
