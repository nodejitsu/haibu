...
var Balancer = exports.Balancer = function (options) {
  ...
  this.getMaster(function (master, conn) {
    master.hook(path.join(__dirname,'..','..','..','balancer'));
  });
...
Balancer.prototype.addApp = function (app, cb) {
  var user = app.user;
...
Balancer.prototype.deactivateApp = function (user, app, cb) {
  var userApps = this.users[user];
  if(!userApps) return cb('User "' + user + '" does not exists.');
  userApps[app.name].active = false;
  return cb();
};
Balancer.prototype.activateApp = function (app, cb) {
  var user = app.user;
  var userApps = this.users[user];
  if(!userApps) return cb('User "' + user + '" does not exists.');
  var name = app.name;
  app = userApps[name];
  if(!app) return cb('User "' + user + '" has no app "' + name +'"');
  app.active = true;
  return cb();
};
Balancer.prototype.destroyApp = function (app, cb) {
  var user = app.user;
  var userApps = this.users[user];
  if(!userApps) return cb('User "' + user + '" does not exists.');
  var name = app.name;
...
Balancer.prototype.addDrone = function (droneid, app, cb) {
  var user = app.user;
  var userApps = this.users[user];
  if(!userApps) return cb('User "' + user + '" does not exists.');
  var name = app.name;
  ...
  var appDrones = app.drones || (app.drones = []);
  appDrones.push(drone);
  return cb();
};
Balancer.prototype.destroyDrone = function (droneid, cb) {
  var drone = this.drones[droneid];
  if(!drone) {
    return cb('Drone "' + droneid + '" does not exists.');
  }
  delete this.drones[droneid];
  var app = drone.app;
  var appDrones = app.drones;
  var l = appDrones.length;
  for(var i = 0; i < l; i++) {
  ...
  var droneProxies = drone.proxies;
  var appProxies = app.proxies;
  for(var desiredPort in droneProxies) {
    var mappings = appProxies[desiredPort];
    if(mappings) {
...
Balancer.prototype.proxies = function (mapping, cb) {
...
Balancer.prototype.proxy = function (droneid, desiredport, actualport, cb) {
