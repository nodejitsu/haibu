...
function logApp(app, message) {
  return [app.user, app.name, message].join(':')
}
...
Spawner.prototype.spawn = function spawn (repo, callback) {
  ...
  fs.stat(repo.startScript, function (err, stats) {
    ...
    // create command line, cant let forever do that because we need a specific sequence of parameters
    // i.e. habu-carapace <haibu-carapace arguments> <script> <script arguments>
    var command = path.join(require.resolve('haibu-carapace'), '..', '..', 'bin', 'carapace');
