...
Spawner.prototype.spawnOptions = function (repo, host, port) {
  var id = ++uid;
  var socket = path.resolve(path.join(__dirname,'..','..','..','running',id+''));
  var options = {
    carapace: path.join(__dirname, '..', '..', '..', 'bin', 'carapace'),
    drone: [
            repo.app.scripts.start.replace(/\.js$/,''),
            '',
            '',
            socket,
            repo.app.name,
            id,
            '',
            haibu.running.bridge.server.server.path
          ]
  };
...
Spawner.prototype.spawn = function (repo, callback) {
  ...
  fs.stat(repo.startScript, function (err, stats) {
    ...
    var env = repo.app.env;
    if (env) {
      for(var key in env) {
        foreverOptions.spawnWith.env[key] = env[key];
      }
    }
    ...
    drone.on('restart', function () {
      var id = options.drone[5];
      if(haibu.running.ports[id]) {
        delete haibu.running.ports[id];
      }
    })
