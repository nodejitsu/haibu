...
haibu.getSpawnOptions = function getSpawnOptions (app) {
  var env = app.env, 
      ...
      nodeVersions = haibu.engineDir && fs.readdirSync(engineDir),
      carapaceDir = haibu.config.get('directories:carapace-installs'),
      carapaceVersions = haibu.engineDir && fs.readdirSync(carapaceDir);
  ...
  if (carapaceVersions) {
    ...
    options.carapaceBin = path.join(carapaceDir, version);
  }
  else {
