...
haibu.getSpawnOptions = function getSpawnOptions (app) {
  ...
  if (nodeVersions) {
    ...
    if (!version) {
      err = new Error('Error spawning drone: no matching engine found');
      err.blame = {
        type: 'user',
  ...
  if (carapaceVersions) {
    ...
    if (!version) {
      err = new Error('Error spawning drone: no matching carapace found');
      err.blame = {
        type: 'user',
        message: 'Repository configuration'
      };
...
Spawner.prototype.spawn = function spawn (repo, callback) {
  ...
  try {
    spawnOptions = haibu.getSpawnOptions(app);
  }
  catch (e) {
    return callback(e);
  }
  ...
  fs.stat(repo.startScript, function (err, stats) {
    if (err) {
      var error = new Error('package.json error: ' + 'can\'t find starting script: ' + repo.app.scripts.start);
      error.blame = {
        type: 'user',
