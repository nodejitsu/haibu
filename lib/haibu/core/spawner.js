...
var Spawner = exports.Spawner = function Spawner(options) {
  ...
  this.engineDir  = options.engineDir  || haibu.config.get('directories:engines');
  this.nodeInstallDir = installDirs.node;
  this.nodeVersions = fs.readdirSync(nodeInstallDir);
...
Spawner.prototype.spawn = function spawn (repo, callback) {
  ...
  foreverOptions = {
    ...
    options:   [path.join(require.resolve('haibu-carapace'), '..', '..', 'bin', 'carapace')].concat(options)
  };
