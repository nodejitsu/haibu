...
Drone.prototype.clean = function (app, callback) {
  ...
  var self = this,
      userDir = path.join(this.appsDir, app.user),
      appDir = path.join(userDir, app.name);
  
  this.stop(app.name, function (err, result) {
    //
    // Ignore errors and continue cleaning
    //
    fs.readdir(userDir, function (err, userApps) {
      if (err) {
        return callback(err);
      }
      
      return userApps.length > 1 
        ? rimraf(appDir, callback)
        : rimraf(userDir, callback);
    });
  });
};
