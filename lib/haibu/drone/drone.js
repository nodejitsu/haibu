...
Drone.prototype.deploy = function (userId, appId, stream, callback) {
  ...
  fs.mkdir(untarDir, '0755', function (err) {
    var child = spawn('tar', ['-xz'], { cwd: untarDir }),
        stderr = '';
    ...
    //
    // Pipe to the `tar` process.
    //
    stream.pipe(child.stdin);
    
    //
    // When the `tar` process has completed, then read the `package.json`
    // file and attempt to start the drone via `this.start()`.
    //
    child.on('exit', function (code) {
      if (code || stderr.length) {
        var err = new Error('tar -xzv exited with code: ' + code);
        err.usage = 'tar -cvz . | curl -sSNT- HOST/drones/deploy/USER/APP';
        err.stderr = stderr;
        return callback(err);
      }
      ...
      haibu.utils.file.readJson(path.join(untarDir, 'package.json'), function (err, pkg) {
        if (err) {
          return callback(err);
        }
...
Drone.prototype.running = function () {
  ...
  haibu.utils.each(this.list(), function (record) {
    var app = record.app;
    haibu.utils.each(record.drones, function (drone) {
      all.push(this.format(app, drone));
    });
  });
