...
Drone.prototype.restart = function (name, callback) {
  ...
  function restartAndUpdate (key, next) {
    ...
    record.drones[key].monitor.once('restart', function (_, data) {
        // _update is now called by listener that is setup in _add
        // there was a problem because the restart event was not being listened for,
        // and when a server crashed and the haibu Drone object was getting out of sync.
        // fixed now. 
        //  now, doing careful testing in morbo.
        processes.push(self._formatRecord(record.drones[data.pid]));
        next();
    });

    //record.drones[key]._expectedRestart = true;
...
Drone.prototype._add = function (app, drone, callback) {
  ...
  // Listen for drone's restart event
  drone.monitor.on('restart', function(_, data) {
    //if (drone._expectedRestart) return;

    self._update(record, pid, data, function () {
    });
    pid = data.pid;
  });
