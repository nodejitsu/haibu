...
Drone.prototype.stop = function (name, cleanup, callback) {
  ...
  function removeAndSave (key, next) {
    function onStop() {
      console.log(app.drones);app.drones[key].monitor.removeListener('error', onErr);
...
Drone.prototype._add = function (app, drone, callback) {
  ...
  function statsUpdate(cb){
  	haibu.common.monitor(drone.pid,function(err,stats){
  	  record.statsTimmer = setTimeout(statsUpdate, 5*1000)
  	  drone.stats=stats
  	  cb&&cb()
  	})
  }
  record.statsTimmer = 0
  ...
  statsUpdate(function(){
  	self._autostartUpdate('add', app, record.drones, callback);
  })
};
