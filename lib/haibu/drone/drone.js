...
//
// ### function _autostartName (app)
// Returns filename for haibu's persistance (autostart) feature
//
Drone.prototype._autostartName = function (app) {
  return haibu.config.get('directories:autostart') + '/' +
         haibu.utils.sanitizeAppname(app.name) + '.json';
};

//
// ### function _autostartUpdate (action, app, drones, callback)
// Updates autostart file if needed
//
Drone.prototype._autostartUpdate = function (action, app, drones, callback) {
  // app.drones = drones count
  var _drones = app.drones,
      autostartName = this._autostartName(app),
  ...
  if (action === 'remove' && dronesCount == 0) {
    fs.unlink(autostartName, callback);
    return;
  }

  // If old autostart file exists
  path.exists(autostartName, function (exists) {
    if (exists) {
      // Read it
      fs.readFile(autostartName, function (err, contents) {
        // If failed - overwrite it
        if (err) return writeFile();
  ...
  function writeFile() {
    fs.writeFile(autostartName, appJson, callback);
  }
};
...
Drone.prototype._remove = function (record, drone, callback) {
  ...
  this._autostartUpdate('remove', app, record.drones, callback);
};
