...
Drone.prototype.start = function (app, callback) {
  ...
  this.spawner.trySpawn(app, function (err, result) {
    ...
    self._add(app, {
      ...
      var response = {
        drone: haibu.utils.clone(result.drone)
      };

      if (self.host) {
        response.drone.host = self.host;
      }
...
Drone.prototype._add = function (app, drone, callback) {
  ...
  drone.monitor.on('start', function() {
    if(self.apps[app.name]) delete self.apps[app.name].ports[pid];
    pid = arguments[0].childData.pid;
  });
  drone.monitor.on('restart', function() {
    delete self.apps[app.name].ports[pid];
    pid = arguments[0].childData.pid;
  });

  function saveDrone (err) {
    self.apps[app.name].drones[pid] = drone;
