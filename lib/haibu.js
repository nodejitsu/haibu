...
haibu.running    = {};
haibu.engineDir  = haibu.config.get('directories:engines');
haibu.nodeInstallDir = haibu.engineDir && haibu.engineDir.node;
haibu.nodeVersions = haibu.nodeInstallDir && fs.readdirSync(haibu.nodeInstallDir);
...
haibu.getSpawnOptions = function getSpawnOptions (app) {
  ...
  if (haibu.nodeVersions) {
    version = semver.maxSatisfying(haibu.nodeVersions, (repo.engines || {}).node);
  }
  if (version) {
    //
